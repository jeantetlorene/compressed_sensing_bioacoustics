...
def calc_back_conv(input_size, conv_layer, dim):
    ...
    if conv_layer.padding == "same" or conv_layer.padding == "valid":
...
class BaseCNN(nn.Module):
    def __init__(
        ...
        if isinstance(self.conv_kernel, int):
            self.conv_kernel = [self.conv_kernel] * self.n_conv_layers

        # Convolutional layers
        self.conv_layers = nn.Sequential()
        self.conv_layers.add_module(
            "conv0",
            nn.Conv2d(n_channels, self.conv_filters, kernel_size=self.conv_kernel[0], padding=self.conv_padding),
        )
        self.conv_layers.add_module("bn0", nn.BatchNorm2d(self.conv_filters)) 
        ...
        for i in range(1, self.n_conv_layers):
            self.conv_layers.add_module(
                f"conv{i}",
                nn.Conv2d(
                    self.conv_filters, self.conv_filters, kernel_size=self.conv_kernel[i], padding=self.conv_padding
                ),
            )
            self.conv_layers.add_module(f"bn{i}", nn.BatchNorm2d(self.conv_filters)) 
