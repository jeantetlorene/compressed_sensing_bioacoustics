...
class CS: 
    ...
    def compress_one_file(self, audio, sample_rate, idx, file_name_no_extension):
        ...
        for start in range(0, audio_windows.shape[0], self.batch_size) :
        ...
        #save in wav
        y_int16 = (compressed_frames * 32767).astype(np.int16)
        file_name = f"{self.folder_compressed_saved}/{file_name_no_extension}_{compressed_frames.shape[0]}_compressed.wav"
        write(file_name, sample_rate, y_int16)
