...
class Evaluation:
    ...
    def _calc_amplitudes_to_predict(self, file_name, preprocessing, Saved_amplitudes=False,  verbose = True):
        if str(
            Path(preprocessing.audio_path, file_name + self.audio_extension)
        ) in glob(str(preprocessing.audio_path / f"*{self.audio_extension}")):
            print("Found file")
        
        init_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-5]
        print(f"{init_time} Load {file_name} data ")
            
        audio_amps, original_sample_rate = preprocessing.read_audio_file(
            str(
                Path(
                    preprocessing.audio_path, file_name + self.audio_extension
                )
            )
        )
        ...
        # Downsample
        init_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-5]
        print(f"{init_time} Downsampling.. ")
        ...
        if Saved_amplitudes : 
            init_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-5]
            print(f"{init_time} Save amplitudes")
            ...
            save_name = Path(self.save_amplitudes_path, file_name + "_amplitudes_to_predict.npy")
    ...
    def _process_one_file(self, file_name, model, preprocessing, threshold, verbose=True):
    ...
    def prediction_files(self, model, threshold, type = "test"):
        ...
        for file in file_names.values:
            ...
            self._process_one_file(file, model, preprocessing, threshold, verbose=True)
    ...
    def _repair_svl(
        self, file_names, preprocessing, annotation_extension, annotation_folder
    ):
        ...
        for file in file_names.values:
            ...
            reader = AnnotationReader(
                self.species_folder, file, annotation_extension, self.audio_extension, self.positive_class,
            )

            audio_amps, original_sample_rate = preprocessing.read_audio_file(
                str(
                    Path(
                        preprocessing.audio_path, file + self.audio_extension
                    )
                )
            )
            ...
            (
                ...
                start_m,
                end_m,
            ) = reader.get_annotation_information_testing(annotation_folder, annotation_extension)
    ...
    def comparison_predictions_annotations(self, folder, type="test"):
        ...
        if os.path.exists(Path(self.species_folder, "Annotations_corrected")):
            ...
            self._repair_svl(
                file_names,
                preprocessing,
                preprocessing.annotation_extension,
        ...
        for file in file_names.values:
            ...
            reader = AnnotationReader(
                self.species_folder,
                file,
                annotation_extension=preprocessing.annotation_extension,
    ...
    def _testing_dataset_run(self, model, type="test", threshold=None, print_report=True, metric="f1",return_image_shape_and_parameters = True):
        data_path = Path(self.species_folder, type)
        
        
        with open(Path(data_path, "X_" + type + "_" + self.audio_extension[1:] + ".pkl"), "rb") as f:
            X = pickle.load(f)
        with open(Path(data_path, "Y_" + type + ".pkl"), "rb") as f:
            Y = pickle.load(f)
    ...
    def _entire_files_run(self, model, type="test", threshold=None, print_report=True, metric="f1",return_image_shape_and_parameters = True):
    
        starting = time.time()
        self.prediction_files(model, type=type, threshold=threshold)
        execution_time = time.time() - starting
        ...
        return F_score, confusion, execution_time
    
    
    def run(self, model, type="test", test_type = "testing_dataset", threshold=None ):
        if test_type == "testing_dataset":
            return self._testing_dataset_run(model, type=type, threshold=threshold)
        
        else : 
            return self._entire_files_run(model, type=type, threshold=threshold)
