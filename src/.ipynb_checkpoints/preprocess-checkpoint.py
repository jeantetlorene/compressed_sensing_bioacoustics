...
from compress import MP3Compression
from compress import AACCompression
from compress import OGGCompression
...
class Preprocessing:
    def __init__(
        ...
        self.compressed_audio_path = Path(self.species_folder, "Compressed_Audio")
    ...
    def create_dataset(self, files_path, dataset_type="train", preprocessing=True, data_augmentation=False, compression_type = "no_compression"):
        ...
        for file in files.values:
            ...
            if str(Path(self.audio_path, file_name_no_extension + self.audio_extension)
            ...
            ###################################################
            AudioSegment.converter = r"C:\Users\gisadmin\Desktop\ffmpeg-master-latest-win64-gpl-shared\bin\ffmpeg.exe"
            if compression_type == "cs":
                print('compressed sensing not done yet')
                #cs_compression = CSCompression(str(Path(self.audio_path,file_name_no_extension + self.audio_extension)))
                #audio_amps, original_sample_rate = cs_compression.compress(self.compressed_audio_path, file_name_no_extension)
            elif compression_type == "aac":
                aac_compression = AACCompression(str(Path(self.audio_path,file_name_no_extension + self.audio_extension)))
                audio_amps, original_sample_rate = aac_compression.compress(self.compressed_audio_path, file_name_no_extension, bitrate= "96k")
                audio_amps, original_sample_rate = aac_compression.compress(self.compressed_audio_path, file_name_no_extension, bitrate= "192k")
                audio_amps, original_sample_rate = aac_compression.compress(self.compressed_audio_path, file_name_no_extension, bitrate= "320k")
                #print('compression loaded from file')
                #audio_amps, original_sample_rate = aac_compression.compress_load_aac_from_file(self.compressed_audio_path / "aac", file_name_no_extension)
            elif compression_type == "mp3":
                mp3_compression = MP3Compression(str(Path(self.audio_path,file_name_no_extension + self.audio_extension)))
                audio_amps, original_sample_rate = mp3_compression.compress(self.compressed_audio_path, file_name_no_extension, bitrate="96k") # optional parameters: preset="fast", threads=4
                audio_amps, original_sample_rate = mp3_compression.compress(self.compressed_audio_path, file_name_no_extension, bitrate="192k")
                audio_amps, original_sample_rate = mp3_compression.compress(self.compressed_audio_path, file_name_no_extension, bitrate="320k")
                #print('compression loaded from file')
                #audio_amps, original_sample_rate = mp3_compression.compress_load_mp3_from_file(self.compressed_audio_path / "mp3", file_name_no_extension)
            elif compression_type == "ogg":               
                ogg_compression = OGGCompression(str(Path(self.audio_path,file_name_no_extension + self.audio_extension)))              
                audio_amps, original_sample_rate = ogg_compression.compress(self.compressed_audio_path, file_name_no_extension, quality=0)
                audio_amps, original_sample_rate = ogg_compression.compress(self.compressed_audio_path, file_name_no_extension, quality=5)
                audio_amps, original_sample_rate = ogg_compression.compress(self.compressed_audio_path, file_name_no_extension, quality=10)
                #print('compression loaded from file')
                #audio_amps, original_sample_rate = ogg_compression.compress_load_ogg_from_file(self.compressed_audio_path / "ogg", file_name_no_extension)
            elif compression_type == "flac":
                flac_compression = FlacCompression(str(Path(self.audio_path,file_name_no_extension + self.audio_extension)))
                #audio_amps, original_sample_rate = flac_compression.compress(self.compressed_audio_path, file_name_no_extension, compression_level=0)
                #audio_amps, original_sample_rate = flac_compression.compress(self.compressed_audio_path, file_name_no_extension, compression_level=6)
                audio_amps, original_sample_rate = flac_compression.compress(self.compressed_audio_path, file_name_no_extension, compression_level=12)
                #print('compression loaded from file')
                #audio_amps, original_sample_rate = flac_compression.compress_load_flac_from_file(self.compressed_audio_path / "flac", file_name_no_extension)
            else: 
                pass
            ###################################################
            
            if preprocessing and compression_type == "no_compression": 
                #preprocessing 
                print("Filtering...") 
                # Low pass filter
